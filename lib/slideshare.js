...
SlideShare.prototype = {
     ...
     * @return {Object} A slideshow object
     */
    getUserSlideshowById: function(id, user, password, callback) {
        var params = this.coreParams();
        params.data.slideshow_id = id;
        params.data.username = user;
     ...
     * @param {string} password password
     * @param {string} id The id of the slideshow
     * @param {string} title Title of the slideshow
     * @param {Object} opts Optional parameters, for example {tags: 'tag', make_slideshow_private: true, generate_secret_url: true}, or null.
     * @return {Object} An object containing an array of Tags
     */
    editSlideshow: function(username, password, id, title, opts, callback) {
        var params = this.coreParams();
        params.data.username = username;
        params.data.password = password;
    	params.data.slideshow_id = id;
        params.data.slideshow_title = title;
        if (opts != null) {
            params.data.slideshow_tags = opts.tags;
            if ('make_slideshow_private' in opts) params.data.make_slideshow_private = (opts.make_slideshow_private ? 'Y' : 'N');
            if ('generate_secret_url' in opts) params.data.generate_secret_url = (opts.generate_secret_url ? 'Y' : 'N');
            if ('allow_embeds' in opts) params.data.allow_embeds = (opts.allow_embeds ? 'Y' : 'N');
            if ('share_with_contacts' in opts) params.data.share_with_contacts = (opts.share_with_contacts ? 'Y' : 'N');
            console.log(params.data);
        }
        rest.get(this.api_url + 'edit_slideshow', params).on('complete', function(data) {
            return callback(data);
        });
    },
     ...
     * @return {Object} An object containing an array of Tags
     */
    deleteSlideshow: function(username, password, id, callback) {
        var params = this.coreParams();
        params.data.username = username;
        params.data.password = password;
    	params.data.slideshow_id = id;
        rest.get(this.api_url + 'delete_slideshow', params).on('complete', function(data) {
            return callback(data);
        });
    },
     ...
     * @param {string} title Title of the slideshow
     * @param {Object} opts Optional parameters, for example {description: 'slideshow description', tags: 'tag', make_src_public: true}, or null.
     * @return {Object} An object containing an array of Tags
     */
    uploadSlideshow: function(username, password, id, title, opts, callback) {
        var params = this.coreParams();
        params.data.username = username;
        params.data.password = password;
    	params.data.slideshow_id = id;
        ...
        if (opts != null) {
            params.data.slideshow_tags = opts.tags;
            params.data.slideshow_description = opts.description;
            if ('make_src_public' in opts) params.data.make_src_public = (opts.make_src_public ? 'Y' : 'N');
            if ('make_slideshow_private' in opts) params.data.make_slideshow_private = (opts.make_slideshow_private ? 'Y' : 'N');
            if ('generate_secret_url' in opts) params.data.generate_secret_url = (opts.generate_secret_url ? 'Y' : 'N');
            if ('allow_embeds' in opts) params.data.allow_embeds = (opts.allow_embeds ? 'Y' : 'N');
            if ('share_with_contacts' in opts) params.data.share_with_contacts = (opts.share_with_contacts ? 'Y' : 'N');
        }
        rest.get(this.api_url + 'upload_slideshow', params).on('complete', function(data) {
            return callback(data);
        });
    }
};
